...
class CROSS_DOMAIN_FSAR(nn.Module):
    ...
    def __init__(self, args):
        ...
        #self.classifier = nn.Linear(self.mid_dim, self.argss.class_num)
    ...
    def forward(self, inputs, iteration=1):  # 获得support support labels, query, support real class
        ...
        # (2)将decoder出来的重构特征, 再次放入encoder中，相当于增加噪声，提高模型的泛化能力
        reconstruct_support_features, reconstruct_target_features = reconstruct_source_domain_features[0:support_num], reconstruct_source_domain_features[support_num:]
        reconstruct_support_features_g_pro, reconstruct_support_features_g, reconstruct_target_features_g = self.temporal_transformer(reconstruct_support_features, reconstruct_target_features, support_labels)
        cum_dist_reconstruct_g = -self.otam_distance(reconstruct_support_features_g_pro, reconstruct_target_features_g)  # 全局对比全局
        #融合的support的prototype， 原始的和加入目标域噪音的
        # (3) 用融合特征来计算距离
        support_features_fused = 0.5 * support_features_g_pro + 0.5 * reconstruct_support_features_g_pro
        target_features_fused = 0.5 * query_features_g + 0.5 * reconstruct_target_features_g
        cum_dist_fused = -self.otam_distance(support_features_fused, target_features_fused)  # 全局对比全局
        ##########################################################################################################
        # 预训练结束之后。进行target的无监督学习，将target的信息加入
        target_self_s_loss = 0
        if self.training and iteration > self.start_cross :
            target_self_s_loss = self.target_domain_self_supervised(target_domain_features_enc)
