...
colors = ['green','darkred','orange','hotpink','blue', 'lime', 'r', 'violet','m','peru','olivedrab','hotpink']#
Label_Com = ['S-1', 'T-1', 'S-2', 'T-2', 'S-3', 'T-3', 'S-4', 'T-4','S-5','T-5', 'S-6', 'T-6', 'S-7','T-7','S-8', 'T-8','S-9','T-9',
...
def visual(X):
    ...
    #tsne = manifold.TSNE(perplexity=10, n_components=3, init='pca', random_state=501)
    X_tsne = tsne.fit_transform(X)

    print("Org data dimension is {}. Embedded data dimension is {}".format(X.shape[-1], X_tsne.shape[-1]))
    #X_tsne =np.delete(X_tsne, 1, axis=1)
    #'''嵌入空间可视化'''
...
def plot_with_labels_(S_lowDWeights, Trure_labels, name):
    plt.cla()  # 清除当前图形中的当前活动轴,所以可以重复利用
...
def draw(x, name, num, labels= [] ):
    ...
    plot_with_labels_(visual(x), y_s, name)
    ...
    plt.savefig('/home/guofei/Projects/w-figure/' + name + '.png', format='png', dpi=300, bbox_inches='tight')
    plt.clf()
    # plt.show(fig)
