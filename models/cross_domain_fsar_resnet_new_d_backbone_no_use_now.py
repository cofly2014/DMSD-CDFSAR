...
import copy
from models.resNet_tmp import MyResNet
...
class CROSS_DOMAIN_FSAR(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.differ_array= []
        self.source_domain_features_save = []
        self.target_domain_features_save = []
    ...
    def forward(self, inputs, iteration=1):  # 获得support support labels, query, support real class
        ...
        self.source_domain_features_save.append(source_domain_features.detach())
        self.target_domain_features_save.append(target_domain_features.detach())


        if iteration % 5 == 0:
            self.source_domain_features_save = torch.stack(self.source_domain_features_save, dim=0).reshape(-1, 8* 512)
            self.target_domain_features_save = torch.stack(self.target_domain_features_save, dim=0).reshape(-1, 8* 512)

            #differ = mmd.compute_mmd_linear( self.source_domain_features_save, self.target_domain_features_save, 50)
            differ = mmd.compute_mmd(self.source_domain_features_save, self.target_domain_features_save,50)
            self.differ_array.append(differ)
            print(differ)
            print("~~~~~~")
            self.source_domain_features_save=[]
            self.target_domain_features_save=[]
        if iteration == 500:
            self.differ_array = torch.stack(self.differ_array, dim = 0)
            differ_mean = torch.mean(self.differ_array, dim=0)
            print(differ_mean)
            print("~~~~~~~~~~~~~")
            self.differ_array =[]
            exit(1)
        ...
        '''
        if iteration % 5 == 0:
            self.source_domain_features_save = torch.stack(self.source_domain_features_save, dim=0).reshape(-1, 512)
            self.target_domain_features_save = torch.stack(self.target_domain_features_save, dim=0).reshape(-1, 512)
            differ = mmd.compute_mmd_rbf( self.source_domain_features_save, self.target_domain_features_save, 0.1)
            self.differ_array.append(differ)
            print(differ)
            print("~~~~~~")
            self.source_domain_features_save=[]
        ...
        #####################################################################################################
        source_domain_features_ori = torch.concat([support_features, query_features], dim=0)
        # 监督学习，对用backbone中提取的源域的数据进行分类
        class_logits_origin = self.classification_layer(source_domain_features_ori.mean(1))
        class_logits = class_logits_origin
    ...
    def forward_test(self, inputs, iteration=1):  # 获得support support labels, query, support real class
        ...
        # (1) metric distance restnet+otam 用来对比测试
        #####################################################################################################
        source_domain_features_ori = torch.concat([support_features, query_features], dim=0)
        # 监督学习，对用backbone中提取的源域的数据进行分类
        class_logits_origin = self.classification_layer(source_domain_features_ori.mean(1))
