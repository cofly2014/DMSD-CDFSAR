...
class CROSS_DOMAIN_FSAR(nn.Module):
    ...
    def forward(self, inputs, iteration=1):  # 获得support support labels, query, support real class
        ...
        query_labels = inputs['target_labels']
        target_domain_set = inputs['target_domain_set']
        target_domain_label = inputs['target_domain_label']
        #######################################################################################################
        ...
        ####draw start ######################################################################################
        #draw
        draw_data = torch.cat([source_domain_features, target_domain_features], dim = 0)
        import numpy as np
        s_labels = torch.ones(source_domain_features.shape[0])
        t_labels = torch.zeros(target_domain_features.shape[0])
        draw_labls = torch.cat([s_labels, t_labels],dim=0)
        draw(draw_data.cpu().reshape(-1, 8 * 512).detach().numpy(), "cross_features_origin", 100, draw_labls)
        ####draw end ################################################################################################

        #####散点分类图###############################################################################################
        #散点画分类图
        draw_data = support_features
        draw(draw_data.cpu().reshape(-1, 8 * 512).detach().numpy(), "Baseline", draw_data.shape[0], support_labels)
        ##############################################################################################################
        ...
        #####散点分类图###############################################################################################
        #散点画分类图
        draw_data = support_features_g
        draw(draw_data.cpu().reshape(-1, 8 * 512).detach().numpy(),  'DMSD', draw_data.shape[0], support_labels)
        ##############################################################################################################
        ...
        ####draw start ##################################################################################
        #draw散点图
        draw_data = torch.cat([source_domain_features_g, target_domain_features_enc], dim = 0)
        import numpy as np
        s_labels = torch.ones(source_domain_features_g.shape[0])
        t_labels = torch.zeros(target_domain_features_enc.shape[0])
        draw_labls = torch.cat([s_labels, t_labels],dim=0)
        draw(draw_data.cpu().reshape(-1, 8 * 512).detach().numpy(), "cross_features", 100, draw_labls)
        ####draw end################################################################################
        ...
        if iteration > self.start_cross:
            ...
            ###draw start###################################################################################
            target_domain_features_enc = self.inner_teacher(target_domain_features, target_domain_features, target_domain_features)
            draw_data = torch.cat([teacheer_source_domain_features_g, target_domain_features], dim=0)
            draw_labls = torch.cat([s_labels, t_labels], dim=0)
            draw(draw_data.cpu().reshape(-1, 8 * 512).detach().numpy(), "mixed_cross_features", 100, draw_labls)
            ####draw end#########################################################################################
    ...
    def forward_test(self, inputs, iteration=1):  # 获得support support labels, query, support real class
        ...
        cum_dist_g = -self.otam_distance(support_features_g_pro, query_features_g)  # 全局对比全局
        ##################################################
        if support_features_g.shape[0] == 100 :
            draw(support_features_g.cpu().reshape(-1, 8 * 512).numpy(), "proto", 100, support_labels)
        ##################################################
        P_global = F.softmax(cum_dist_g, dim=-1)
        # 监督学习，对用backbone中提取的源域的数据进行分类
        class_logits = self.classification_layer(source_domain_features_g.mean(1))
