...
def verify_checkpoint_dir(checkpoint_dir, resume, iteration):
    if resume:  # verify that the checkpoint directory and file exists
        ...
        if iteration == 0:
            ...
            checkpoint_file = os.path.join(checkpoint_dir, 'checkpoint{}.pt'.format(iteration))
...
def get_log_files(checkpoint_dir, resume, iteration):
    """
    ...
    """
    verify_checkpoint_dir(checkpoint_dir, resume, iteration)
...
def aggregate_accuracy(test_logits_sample, test_labels):
    ...
    """
    test_logits_sample = test_logits_sample.reshape(-1, test_logits_sample.shape[0], test_logits_sample.shape[1])
...
def aggregate_accuracy_sta(test_logits_sample, test_labels, real_target_labels, statics):
    """
    Compute classification accuracy.
    """
    test_logits_sample = test_logits_sample.reshape(-1, test_logits_sample.shape[0], test_logits_sample.shape[1])
    ...
    for i in test_labels.tolist():
        real_lable = real_target_labels[i].int().item()
        if real_lable in statics:
           result = torch.eq(test_labels[i], torch.argmax(averaged_predictions[i], dim=-1))
           statics[real_lable].append(result)
        else:
          statics[real_lable] = []
          result = torch.eq(test_labels[i], torch.argmax(averaged_predictions[i], dim=-1))
          statics[real_lable].append(result)
...
def timewise_cos1(x,y):
    ...
    all_distances_tensor = torch.zeros(m, n).cuda()
    for i in range(n):
...
def EuclideanDistance(t1,t2):
    ...
    if dim==2:
        ...
        B,N,_=t1.size()    # 125 8 1024
        _,M,_=t2.size()    # 125 8 1024
        dist = -2 * torch.matmul(t1, t2.permute(0, 2, 1)) + torch.sum(t1 ** 2, -1).view(B, N, 1)  + torch.sum(t2 ** 2, -1).view(B, 1, M)
...
def Euclidean_Distance(t1,t2):
    dim = len(t1.size())
    temp = torch.sqrt(torch.sum(t1 ** 2, -1) + torch.sum(t2 ** 2, -1))
    temp =  torch.sum(torch.sum(temp, dim=0),  dim =0)
    temp = temp/(t1.shape[0]*t1.shape[1])
    return temp
...
def getcombinations(allclass,num):
    from itertools import combinations
    combins = [c for c in  combinations(range(allclass), num)]
    return combins
...
def send_email(token,text,touser):
    smtp_obj = smtplib.SMTP('smtp.qq.com',587)
    smtp_obj.login('769038835@qq.com',token)
 
    mail_text = text
    msg_body = MIMEText(mail_text,'plain','utf-8')
    msg_body['From'] = Header('robot<769038835@qq.com>')  #发送人->参考RFC2047, RFC822协议
    msg_body['Subject'] = Header('message','utf-8')  #内容主题
 
    smtp_obj.sendmail('769038835@qq.com',touser,msg_body.as_string())