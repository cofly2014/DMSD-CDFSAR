...
def parse_command_line():
    ...
    parser.add_argument("--source_dataset", choices=["ssv2", "ssv2_cmn", "kinetics", "hmdb", "ucf", "diving48", "rareAct"], default="rareAct", help="Dataset to use.")
    parser.add_argument("--target_dataset", choices=["ssv2", "ssv2_cmn", "kinetics", "hmdb", "ucf", "diving48", "rareAct"], default="diving48",       help="Dataset to use.")

    parser.add_argument("--checkpoint_dir", "-c", default="./checkpoint_(hmdb_source)_diving48/", help="Directory to save checkpoint to.")
    parser.add_argument("--test_model_path", "-m", default="./checkpoint_(hmdb_source)_diving48/", help="Path to model to load and test.")

    parser.add_argument("--resume_checkpoint_iter", type=int, default=11200, help="Path to model to resume.")
    ...
    parser.add_argument('--class_num', type=int, default=66, help='the number of class catetories in the training stage')
    ...
    if args.source_dataset == "kinetics":
        ...
        args.scratch_data = "/home/guofei/mydata/video-origin-frames/"
        args.path = os.path.join(args.scratch_data, "kinetics-frames/train_400/")
    elif args.source_dataset == "hmdb":
        args.scratch_data = "/mnt/mydata/video-mini-frames/"
        args.trainlist = os.path.join(args.scratch, "splits/hmdb_ARN")
        args.path = os.path.join(args.scratch_data, "HMDB_51/")
    elif args.source_dataset == "ssv2":
        args.scratch_data = "/mnt/mydata/video-mini-frames/"
        args.scratch_data = "/data1/wangq/data/video-mini-frames/"
        args.trainlist = os.path.join(args.scratch, "splits/ssv2_OTAM")
        args.path = os.path.join(args.scratch_data, "mini-ssv2-frames-number/")
    elif args.source_dataset == "ucf":
        args.scratch_data = "/mnt/mydata/video-mini-frames/"
        args.trainlist = os.path.join(args.scratch, "splits/ucf_ARN")
        args.path = os.path.join(args.scratch_data, "UCF101_frames/")
    elif args.source_dataset == "diving48":
...
class Learner:
    ...
    def train_task(self, task_dict, iteration):
        ...
        if iteration > self.args.start_cross:
            ...
            lambdas = [1, 1, 1, 0.2, 0.2]  #upervied+meta + meta 和 supervised蒸馏
